...
if (command.contains("No such file or directory"))
   {
    logfile("busybox install started");
    cstring = getadb() + " shell mkdir -p /data/local/tmp/adbfire";
    command=RunProcess(cstring);
   }

  cstring = getadb() + " shell ls /data/local/tmp/adbfire/busybox";
  command=RunProcess(cstring);
  ...
  if (command.contains("No such file or directory"))
     {
      ...
      QString cstring = getadb() + " push "+busybox+ " /data/local/tmp/adbfire/";
      QString command=RunProcess(cstring);
     ...
     else
      {
          ...
          cstring = getadb() + " shell chmod 755 /data/local/tmp/adbfire/busybox";
          command=RunProcess(cstring);

          // logfile(cstring);
          logfile(command);

           cstring = getadb() + " shell /data/local/tmp/adbfire/busybox --install -s /data/local/tmp/adbfire";
           command=RunProcess(cstring);
...
cstring = getadb() + " shell ls /data/local/tmp/adbfire/mntdrives.sh";
command=RunProcess(cstring);
...
// extra rms
 ...
 if (!dir.isEmpty() )
 {
   ...
   if (!QDir(dir+"/userdata").exists() )
     ...
     {
       QMessageBox::critical(0,"","Invalid backup folder!\nuserdata folder not found or invalid." );
       logfile("Invalid backup folder!\nuserdata folder not found or invalid.");
       return;
     }
...
void MainWindow::on_actionInstall_busybox_triggered()
{
   ...
   QMessageBox::StandardButton reply;
         reply = QMessageBox::question(this, "", "Install System Tools?",
            QMessageBox::Yes|QMessageBox::No);
    ...
    QString cstring = getadb() + " shell ls /data/local/tmp/adbfire";
    QString command=RunProcess(cstring);
      ...
      command=RunProcess(cstring);
         ...
         if (!command.contains("bytes"))
           { QMessageBox::critical(
            this,
             "",
             "file1: system tools install failed ");
             logfile("file1: system tools install failed ");
             // logfile(cstring);
              logfile(command);
              ui->progressBar->setHidden(true);
             return;
         ...
         logfile(command);
               ...
               cstring = getadb() + " push "+busybox3+ " "+busypath;
               command=RunProcess(cstring);
               ...
               if (!command.contains("bytes"))
                  { QMessageBox::critical(
                               this,
                               "",
                               "file3: system tools install failed");
                   logfile("file3: system tools install failed ");
                   // logfile(cstring);
                   logfile(command);
                   ui->progressBar->setHidden(true);
                   return;
               ...
               logfile(command);
                ...
                cstring = getadb() + " push "+busybox4+ " "+busypath;
                command=RunProcess(cstring);
                ...
                if (!command.contains("bytes"))
                   { QMessageBox::critical(
                                this,
                                "",
                                "file4: system tools install failed");

                    logfile("file4: system tools install failed ");
                    // logfile(cstring);
                    logfile(command);
                    ui->progressBar->setHidden(true);
                    return;
                ...
                logfile(command);
                 ...
                 if (!command.contains("bytes"))
                    { QMessageBox::critical(
                                 this,
                                 "",
                                 "file5: system tools install failed");

                     logfile("file5: system tools install failed ");
                     // logfile(cstring);
                     logfile(command);
                     ui->progressBar->setHidden(true);
                     return;
     ...
     logfile(command);
      ...
      logfile(command);
        ...
        else
        {
         ...
         cstring = getadb() + " shell su -c mkdir -p /system/etc/init.d";
         command=RunProcess(cstring);
         logfile(command);
         ...
         cstring = getadb() + " shell su -c cp /data/local/tmp/adbfire/install-recovery*  /system/etc/";
         command=RunProcess(cstring);
         logfile(command);
        ...
        cstring = getadb() + " shell su -c cp /data/local/tmp/adbfire/init.bueller.sh  /system/etc/";
        command=RunProcess(cstring);
        logfile(command);
...
void MainWindow::on_actionUninstall_Busybox_triggered()
{
   ...
   QString cstring = getadb() + " shell ls /data/local/tmp/adbfire/mntdrives.sh";
   QString command=RunProcess(cstring);
...

       {
         ...
         QFile file(commstr);
           ...
           if(!file.open(QFile::WriteOnly))
             ...
             QTextStream out(&file);
             out  << "#!/bin/sh" << endl;
             out  <<  "ssh -p 2222 root@"+daddr  << endl;
...

       {
         ...
         QFile file(commstr);
           ...
           //  if(!file.open(QFile::WriteOnly |
             ...
             QTextStream out(&file);
             out  << "#!/bin/sh" << endl;
             out  << "sftp -P 2222 root@"+daddr  << endl;
...

{
   ...
   // if (!isConnected && !isusb)
    ...
    //cstring=getadb()+" shell /data/local/tmp/adbfire/busybox find /storage/ -type d -maxdepth 2 -perm 0771";
    //command=RunProcess(cstring);
...
void MainWindow::on_Install_v2Recovery_triggered()
{
...
void MainWindow::load_filemanager(QString dir)
{
    ...
    cstring=getadb()+shell+"/data/local/tmp/adbfire/busybox find " +'"'+dir+'"'+"  -type d -maxdepth 1";
    command=RunProcess(cstring);
    logfile(command);


    cstring=getadb()+shell+"/data/local/tmp/adbfire/busybox find " +'"'+dir+'"'+"  -type f -maxdepth 1";
    command=command+RunProcess(cstring);
 logfile(command);
    QStringList dirlist = command.split("\n");

     dirlist = dirlist.filter(QRegularExpression("^(?!\s*$).+"));  // remove blanks
     dirlist = dirlist.filter(QRegularExpression("^(?!find:)"));   // remove find:

     ui->directoryWidget->addItems(dirlist);
     ui->directoryWidget->insertItem(0,"..");


      s = ui->directoryWidget->item(1)->text();

      s = s.trimmed();

      if (s == "/")
        {
         ui->directoryWidget->insertItem(2,"/data/local/tmp/");
         ui->directoryWidget->insertItem(2,"/sdcard/");
     }
...
void MainWindow::on_directoryWidget_doubleClicked(const QModelIndex &index)
{
       QString itemstring;
       int row = index.row();
      ...
      if (!is_su())
       ...
       itemstring = index.data().toString();

       int z = itemstring.lastIndexOf("/");
       int x = itemstring.length();

       QString itembase2 = itemstring.mid(z+1,x);
       QString itembase1 = itemstring.mid(0,z+1);


       cstring=getadb()+shell+"/data/local/tmp/adbfire/busybox find "+itembase1+" -name "+itembase2+ " -type f -maxdepth 1";
       command=RunProcess(cstring);

       if (command.contains(itemstring))
        {

           pull_single_file(itemstring.simplified() );
       ...
       if (row == 0)
         ...
         itemstring = ui->directoryWidget->item(1)->text();

         int r = itemstring.lastIndexOf("/");
          QString temp = itemstring.left(r);

          //QMessageBox::critical(0,"",temp);

          r = temp.lastIndexOf("/");
          itemstring = temp.left(r);
...
void MainWindow::on_dirbackButton_clicked()
{
...
switch(ui->directoryBox->currentIndex() ){
case 0:
xpath = mcpath+"/addons/";
break;
...
QString cname = ui->directoryBox->currentText();
...

{
 ...
 load_filemanager(xpath);
}


//////////////////////////////////////////////////

void MainWindow::pull_single_file(QString filestring)
{

    QString cstring;
    QString command;

    //QMessageBox::critical(0,"",filestring);


    cstring = getadb() + " pull "+'"'+filestring+'"'+" "+'"'+pulldir+'"';
    command=RunProcess(cstring);
    logfile(command);
    //QMessageBox::critical(this,"",command);
...
void MainWindow::on_customdirButton_clicked()
{
...
void MainWindow::on_directoryWidget_clicked(const QModelIndex &index)
{
   ...
   if (!is_su())
    ...
    if (ui->directoryWidget->item(row)->isSelected())
        { ui->directoryWidget->item(row)->setForeground(Qt::white);
             ui->directoryWidget->item(row)->setBackground(Qt::blue);
        }
    else
        { ui->directoryWidget->item(row)->setForeground(Qt::black);
          ui->directoryWidget->item(row)->setBackground(Qt::white);
        }


}

void MainWindow::on_fm_pullButton_clicked()
{


    if (ui->treeView->hasFocus())
        QMessageBox::critical(0,"","local");

    if (ui->directoryWidget->hasFocus())
        QMessageBox::critical(0,"","remote");
    ...
    QMessageBox::StandardButton reply3;
      reply3 = QMessageBox::question(this, "", "Pull selected file(s)",
                                    QMessageBox::Yes|QMessageBox::No);
      if (reply3 == QMessageBox::No)
          return;
    ...
    for (int i = 0; i < ui->directoryWidget->count(); i++) {
        ...
        if (ui->directoryWidget->item(i)->isSelected())
           {

            pullitem = ui->directoryWidget->item(i)->text();
            pullitem = pullitem.simplified();

                    cstring = getadb() + " pull "+'"'+pullitem+'"'+" "+'"'+pulldir+'"';
                        command=RunProcess(cstring);
...
void MainWindow::on_fm_pushButton_clicked()
{
if (ui->treeView->hasFocus())
    QMessageBox::critical(0,"","local");
...
void MainWindow::on_fm_delButton_clicked()
{
    ...
    QMessageBox::StandardButton reply3;
      reply3 = QMessageBox::question(this, "", "Delete selected file(s)",
                                    QMessageBox::Yes|QMessageBox::No);
      if (reply3 == QMessageBox::No)
          return;
        ...
        if (ui->directoryWidget->item(i)->isSelected())
           {
            ...
            delitem = ui->directoryWidget->item(i)->text();
            delitem = delitem.simplified();

            cstring = getadb() + " shell rm -r "+'"'+ delitem +'"';
            command=RunProcess(cstring);
            logfile(cstring);
            logfile(command);
            }
...
void MainWindow::on_fm_mkdirButton_clicked()
{
